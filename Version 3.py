...
def tour_de_jeu(joueur, grille_adverse, grille_visibles_adverses, coups_joues, dernier_tir=None):
    ...
    while True:
        if joueur == 1:
            ...
            if dernier_tir and direction_index < len(directions):
                d_ligne, d_colonne = directions[direction_index]
                ligne, colonne = dernier_tir[0] + d_ligne, dernier_tir[1] + d_colonne

                if not (0 <= ligne < 10 and 0 <= colonne < 10) or (ligne, colonne) in coups_joues:
                    direction_index += 1
                    continue
            else:
                ligne = random.randint(0, 9)
...
joueur_actuel = 1
dernier_tir_ordinateur = None
historique_tirs = []  # Historique des tirs pour revenir en arrière si nécessaire

while True:
    if joueur_actuel == 1:
        victoire, _ = tour_de_jeu(1, grille_j2, grille_visibles_j1, coups_j2)
        if victoire:
            break
        joueur_actuel = 2
    else:
        if dernier_tir_ordinateur:
            historique_tirs.append(dernier_tir_ordinateur)  # Enregistrer les tirs réussis
        victoire, dernier_tir_ordinateur = tour_de_jeu(2, grille_j1, grille_visibles_j2, coups_j1, dernier_tir_ordinateur)

        if not dernier_tir_ordinateur and historique_tirs:
            # Revenir à la dernière case touchée si nécessaire
            dernier_tir_ordinateur = historique_tirs.pop()
